 ...
 * @Controller
 * 
 * @RequestMapping("/admin") public class AdminCategoryController_old_bak2 {
 * 
 ...
 * if(subCateList.size()!=0) { for(int j=0;j<subCateList.size();j++) {
 * Subcategory sub = subCateList.get(j); json.put("subcategory_id",
 * sub.getSubcategory_id()); json.put("sub_topcategory_id",
 * sub.getTopcategory().getTopcategory_id()); json.put("subcate_name",
 * sub.getSubcate_name()); } } }
 * System.out.println("카테고리 전체목록 요청 시 jsonArray에 잘 담겼나 확인 :"+jsonArray.size());
